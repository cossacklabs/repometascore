...
class ClientTransaction:
    ...
    def __init__(self, home_page_response: Union[bs4.BeautifulSoup], session=None):
    ...
    async def get_indices(self, home_page_response=None):
        key_byte_indices = []
        ...
        if on_demand_file:
            ...
            on_demand_file_response = await self.session.request(method='GET', url=on_demand_file_url)
            on_demand_file_response_content = await on_demand_file_response.text()
            key_byte_indices_match = INDICES_REGEX.finditer(
                str(on_demand_file_response_content))
    ...
    def validate_response(self, response: Union[bs4.BeautifulSoup]):
